...
import io.github.graphdsl.codegen.utils.refs
...
class KmConstructorWrapper(
    ...
    init {
        ...
        val refSet = mutableSetOf<KmName>()
        refSet.addAll(annotations.refs)
        visibleParameterAnnotations.values.forEach { it ->
            it.forEach {
    ...
    fun visibleParameterAnnotationsAttribute(cp: ConstPool): ParameterAnnotationsAttribute? {
        val annotations =
            constructor.valueParameters
                .map {
                    visibleParameterAnnotations[JavaIdName(it.name)]?.map { annotation -> cp.asCtAnnotation(annotation) }
                        ?.toTypedArray()
                        ?: emptyArray()
                }.toTypedArray()
