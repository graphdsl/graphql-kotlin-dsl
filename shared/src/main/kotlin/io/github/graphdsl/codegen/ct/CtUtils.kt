...
import io.github.graphdsl.codegen.utils.*
...
internal fun nullabilityParamAnnotationsAttribute(
    ...
    val annotations = paramTypes.map { it -> it.nullabilityAnnotation?.let { listOf(it) } ?: emptyList() }
...
internal val KmPropertyWrapper.fieldJvmSignature: String get() {
    return when (val signatureType = this.inputType.jvmSignature) {
...
internal val KmType.jvmClassType: SignatureAttribute.ClassType get() =
    when {
        arguments.isEmpty() -> SignatureAttribute.ClassType(kmToJvmBoxedName(name))
